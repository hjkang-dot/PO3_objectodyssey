...
from unified_app.app.models import CharacterSheet
from unified_app.app.prompts import character_sheet_prompt
from unified_app.app.services.gemini_service import GeminiService
from unified_app.app.utils import safe_json_loads
...
def fallback_character_sheet(vision_result: dict[str, Any], parent_input: dict[str, Any]) -> dict[str, Any]:
    ...
    original_object = original_object_hint or (str(objects[0]) if objects else "알 수 없는 친구")
    name = str(parent_input.get("name") or "코코")
    job = str(parent_input.get("job") or "별들의 친구")
    personality = str(parent_input.get("personality") or "따뜻하고 용감한")
    goal = str(parent_input.get("goal") or "새로운 것을 발견하고 싶어 해요")
