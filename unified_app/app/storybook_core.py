...
from unified_app.app.models import ALLOWED_STORY_TONES, CharacterSheet, StoryPackageResponse
from unified_app.app.services.openai_service import OpenAIService
from unified_app.app.utils import PROJECT_ROOT, safe_json_loads
...
def build_common_story_rules() -> str:
    ...
    return """
당신은 5~7세 아동을 위한 창의적이고 임팩트 있는 동화 작가이자, 정확한 형식을 출력하는 데이터 엔지니어입니다. 아이들이 지루해하지 않도록 **짧고 강렬한 대사와 생동감 넘치는 표현**을 사용하여 맞춤형 동화를 생성해야 합니다.

[공통 규칙]
- **이야기는 반드시 한국어로만** 작성하며, 영단어를 절대로 포함하지 않는다.
- 반드시 JSON만 반환한다. 마크다운, 코드블록, 설명문 등은 제외한다.
- JSON 키는 정확히 `title`, `story_pages`, `tts_script`, `choices` 만 사용한다.
- **짧고 강렬한 스토리**: 서술적인 긴 설명(지문)은 줄이고, 주인공의 **생생한 대사와 짧고 역동적인 행동 묘사** 위주로 구성한다.
- 각 페이지(`text`)는 **2~3문장**으로 제한하여 아이들이 한눈에 읽고 듣기 좋게 만든다. 
- **의성어/의태어 필수**: 매 페이지마다 아이들의 흥미를 끄는 소리나 모양 묘사(예: 쌩쌩, 번쩍, 으랏차차)를 1~2개 포함한다.
- **기승전결 5페이지 구성**:
  * 1페이지: 주인공의 등장과 호기심을 자극하는 첫 사건! (예: "어? 이게 뭐지?")
  * 2페이지: 모험의 시작! 신나고 생기 넘치는 출발 묘사.
  * 3페이지: 깜짝 놀랄 만한 장애물이나 귀여운 문제 발생.
  * 4페이지: 주인공의 기발한 생각이나 친구의 도움으로 문제를 **번쩍** 해결!
  * 5페이지: 가슴 따뜻하고 웃음이 나는 깔끔한 마무리.
- **가장 중요(필수)**: 대사(`" "`)가 포함된 문장이나 의성어/의태어는 앞뒤로 **빈 줄(`\n\n`)을 직접 JSON 값 내에 포함**하여 본문과 시각적으로 확실하게 분리한다.
  * (예시: `"text": "주인공이 신나게 뛰어가요.\n\n\"와아! 정말 신난다!\"\n\n오늘 밤은 멋진 꿈을 꿀 것 같아요."`)
- 모든 문장의 끝(`.`, `!`, `?`)에는 줄 바꿈(`\n`)을 추가하여 가독성을 높인다.
- `tts_script`는 전체 문장을 담은 배열이며, 각 항목은 `line`과 `tone`(생동감 넘치는 낭독 지시어)으로 구성한다.
- 각 page의 `image_prompt`는 상세한 영문 프롬프트로 작성하며, 일관된 캐릭터 외형과 'no text' 규칙을 준수한다.
""".strip()
...
# - 문장은 쉽고 또렷하게 쓰되 너무 단조롭지 않게 리듬을 살린다.
# - 아이가 듣는 동안 집중할 수 있도록 의성어와 의태어를 적절히 사용한다.
# - 의성어와 의태어의 위, 아래에는 공백을 추가하여 시각적으로 구분되게 한다.
...
- 갈등이 있더라도 무겁지 않게 만들고, 안심할 수 있는 감정선으로 회복시킨다.
...
def _build_fallback_sentences(character_sheet: dict[str, Any], story_tone: str) -> list[list[str]]:
    ...
    return [
        ...
        [
            f"{name}는 서두르지 말하자고 스스로 다독이며 주변을 찬찬히 살폈어요.",
...
def _resolve_reference_path(reference_image: str) -> Path:
    """상대경로나 절대경로, 또는 URL 형태(/static/...)로 들어온 reference image 경로를 실제 파일 경로로 맞춘다."""
...
def _build_story_cover_prompt(base_style_prompt: str, character_sheet: dict[str, Any], title: str) -> str:
    ...
    return (
        f"{base_style_prompt} "
        f"Create a high-quality, full-scene children's storybook cover illustration for the title '{title}'. "
        f"The illustration must feature {name} (an object-based character inspired by {original_object}) "
        f"positioned within a rich, detailed background environment that represents the theme of {job} and the goal of {goal}. "
        f"Preserve these key visual traits: {traits}. "
        "The composition should be iconic and cinematic, with professional foreground-background separation, "
        "atmospheric lighting, and a polished picture-book aesthetic. "
        "Ensure there is a sense of space and wonder, making it look like a complete professional book cover. "
        "Keep enough visual breathing room for a title at the top or bottom, but do not include any text, "
        "letters, words, logos, or page numbers in the artwork."
    ).strip()
...
def _generate_story_page_images(
    ...
    def generate_cover():
        ...
        cover_image_path = gemini_service.generate_image(
            cover_final_prompt,
            str(reference_path),
            "story_cover",
        )
        return cover_final_prompt, cover_image_path

    print(f"[DEBUG] Starting sequential image generation for 5 pages + 1 cover...")
    
    # 1. 페이지 이미지 하나씩 순차적으로 생성 (503 에러 방지)
    enriched_pages = []
    for page in story_package["story_pages"]:
        print(f"[DEBUG] Generating image for page {page['page_number']}...")
        enriched_page = generate_single_page(page)
        enriched_pages.append(enriched_page)
        # API 과부하 방지를 위한 2초 지연
        time.sleep(2.0)
        
    # 2. 표지 이미지 생성
    print(f"[DEBUG] Generating cover image...")
    cover_final_prompt, cover_image_path = generate_cover()

    print(f"[DEBUG] Sequential image generation completed.")

    enriched_story = dict(story_package)
    enriched_story["story_pages"] = sorted(enriched_pages, key=lambda x: x["page_number"])
    enriched_story["story_paragraphs"] = [page["page_text"] for page in enriched_story["story_pages"]]
    enriched_story["story_image_style"] = style_key
    enriched_story["cover_prompt"] = cover_final_prompt
    enriched_story["cover_image_path"] = cover_image_path
...
# ---------------------------------------------------------
# [추가] 분할 요청(Granular Request)을 위한 신규 함수들
# ---------------------------------------------------------

def generate_story_text_only(
    character_sheet: dict[str, Any],
    extra_prompt: str = "",
    story_tone: str | None = None,
) -> dict[str, Any]:
    """이미지 생성 없이 동화의 텍스트 구성(JSON)만 먼저 생성한다."""
    ...
    # 텍스트 정보만 반환
    story_package["tone"] = resolved_tone
    return story_package


def generate_single_story_image(
    item_type: str,  # "page" or "cover"
    page_data: dict[str, Any] | None,  # page_number, page_text, image_prompt 포함
    story_title: str,
    character_sheet: dict[str, Any],
    ...
    """특정 페이지나 표지의 이미지 1장만 생성하여 경로를 반환한다."""
    ...
    if item_type == "cover":
        scene_prompt = _build_story_cover_prompt(base_style_prompt, character_sheet, story_title)
        filename = "story_cover"
    else:
        if not page_data: raise ValueError("Page data is required for page image generation.")
        scene_prompt = _build_story_scene_prompt(base_style_prompt, page_data, story_title)
        filename = f"story_page_{page_data['page_number']}"
